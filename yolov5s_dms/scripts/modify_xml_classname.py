...
"""
functions: 更改原有xml中错误类别名字为新类别名字并保存
"""
try:
...
cnt = 0
base = ['left_hand', 'left-hand', 'left hand', 'right hand', 'right_hand', 'right-hand']
for img in img_name:
    ...
    xml = open((dst_xml_dir + '/' + img  + '.xml'), 'w')

    xml.write('<annotation>\n')
    xml.write('\t<folder>' + 'VOC2007' + '</folder>\n')
    xml.write('\t<filename>' + img + '.jpg' + '</filename>\n')
    xml.write('\t<source>\n')
